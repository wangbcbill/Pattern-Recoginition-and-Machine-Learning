...
class Ada_Weak_Classifier(Weak_Classifier):
	...
	def calc_error(self, weights, labels):
		...
		######## TODO ########
		######################
		f_val=np.asarray(self.activations)
		thresh=np.linspace(np.min(f_val), np.max(f_val), self.num_bins)
		err_rate=np.zeros(self.num_bins)
		polarity=np.zeros(self.num_bins)
		for i in range(self.num_bins): 
		    h_val=np.zeros_like(f_val)
		    polarity[i]=1
		    h_val=np.sign(f_val-thresh[i])
		    err_rate[i]=np.sum(np.asarray(weights)[h_val!=np.asarray(labels)])
		    if err_rate[i]>0.5:
		        polarity[i]=-1
		        h_val=-np.sign(f_val-thresh[i])
		        err_copy=np.copy(err_rate[i])
		        err_rate[i]=1-err_copy
		min_idx=np.argmin(err_rate)
		error=err_rate[min_idx]
		self.threshold=thresh[min_idx]  
		self.polarity=polarity[min_idx] 
		return error
